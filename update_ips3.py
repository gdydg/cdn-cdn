...
CNAME_API_URL = 'https://example.com/best_cname.txt' # <-- 请将这里替换为您的 API 地址
...
def get_cname_target_from_api():
    """从 API 获取优选 CNAME 目标"""
    print(f"正在从 {CNAME_API_URL} 获取优选 CNAME...")
    try:
        response = requests.get(CNAME_API_URL, timeout=10)
...
def delete_dns_record(record):
    ...
    try:
        ...
        print(f"成功删除旧的 CNAME 记录: {record.id} (指向 {record.records[0]})")
        return True
    except exceptions.ClientRequestException as e:
        print(f"错误: 删除记录 {record.id} 时失败: {e}")
        return False

def create_cname_record(cname_target):
...
def main():
    """主执行函数"""
    print("--- 开始从 API 更新华为云 CNAME 解析记录 (仅限全网默认线路) ---")
    
    if not all([DOMAIN_NAME]):
        print("错误: 缺少必要的 DOMAIN_NAME 环境变量。")
        return

    # 从 API 获取最新的 CNAME 目标
    new_cname_target = get_cname_target_from_api()
    if not new_cname_target:
        print("未能从 API 获取 CNAME 目标，任务终止。")
        return
    ...
    if existing_record:
        if existing_record.records and existing_record.records[0] == new_cname_target:
            print(f"\n记录已是最新，无需更新。当前指向: {new_cname_target}")
            print("--- 任务完成 ---")
            return
        else:
            print("\n--- 记录值不正确或已变更，开始删除旧记录 ---")
            delete_dns_record(existing_record)
    else:
        print("\n无需删除旧记录。")

    print("\n--- 开始创建新的 CNAME 记录 ---")
    if create_cname_record(new_cname_target):
        print(f"\n--- 更新完成 ---")
    else:
        print(f"\n--- 更新失败 ---")
